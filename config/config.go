...
type ServiceConfig struct {
	...
	//
	// Because ReadTimeout does not et Handlers make per-request
...
/*
func (cfg *ServiceConfig) UpdateHost() gin.HandlerFunc{
 return func(c *gin.Context) {
	CurrentAddressTable := map[string]string{}
	cleaner := NewURIParser()
	...
	for _, j := range cfg.Endpoints{
		...
		if k == 0 {
			...
			data = append(data,cleaner.CleanHost(n))
			}
			copy(i.Host,data)
			fmt.Println(i.Host)
		}
